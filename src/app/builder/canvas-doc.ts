export function generateCanvasDocument(): string {
  ...
  const script = [
    ...
    '  if(d.type==="UPDATE_VIDEO_URL"){',
    '    var vblk=document.querySelector("[data-block-id=\\""+d.payload.id+"\\"]");',
    '    if(vblk){',
    '      var vif=vblk.querySelector("iframe");',
    '      if(vif){',
    '        var vurl=d.payload.url;',
    '        var vm=vurl.match(/(?:youtube\\.com\\/watch\\?(?:.*&)?v=)([^&]+)/);',
    '        if(vm)vurl="https://www.youtube.com/embed/"+vm[1];',
    '        else{var vs=vurl.match(/youtu\\.be\\/([^?&]+)/);if(vs)vurl="https://www.youtube.com/embed/"+vs[1];}',
    '        vif.src=vurl;',
    '      }',
    '    }',
    '  }',
